...
go_server_port="${AEP_GO_SERVER_PORT:-4320}"
...
go build -o "${work_directory}/aep-interop" ./cmd/aep-interop
(
  cd "${node_repository}"
  corepack pnpm build
)
...
"${work_directory}/aep-interop" agent \
  -platform-url "http://127.0.0.1:${node_platform_port}" \
  -service-url "http://127.0.0.1:${node_service_port}" \
  >"${work_directory}/go-agent-node.json"

cleanup
processes=()

"${work_directory}/aep-interop" server -listen "127.0.0.1:${go_server_port}" \
  >"${work_directory}/go-server.log" 2>&1 &
go_server_process=$!
processes+=("${go_server_process}")

wait_for_url "http://127.0.0.1:${go_server_port}/health" "${go_server_process}" "Go Server" "${work_directory}/go-server.log"

PLATFORM_URL="http://127.0.0.1:${go_server_port}" \
  SERVICE_URL="http://127.0.0.1:${go_server_port}" \
  node "${node_repository}/examples/aep-agent-did-web-grant-status-revoke/dist/index.js" \
  >"${work_directory}/node-agent-go.json"

node "${repository_root}/scripts/verify-node-interoperability.mjs" \
  "${work_directory}/go-agent-node.json" \
  "${work_directory}/node-agent-go.json" \
  "${output_directory}/aep-go-node-interoperability.json"

echo "Interoperability report: ${output_directory}/aep-go-node-interoperability.json"
